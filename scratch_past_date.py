...
hourly_vars = (m.HOURLY_VARS + list(m.BAND_VARS.values()) + list(m.HUMIDITY_VARS.values())
               + [wind_speed_var, wind_dir_var, m.FREEZING_LEVEL_VAR, f"cloudcover_{wind_hpa}hPa"]
...
day_scores = m.compute_day_scores(forecast, mtn, wind_speed_var, wind_hpa)
if TARGET_DATE in day_scores:
    m.print_day_score_table(mtn, {TARGET_DATE: day_scores[TARGET_DATE]})

rows = m.build_hourly_rows(forecast, mtn, wind_speed_var, wind_dir_var, summit_hpa=wind_hpa)
