...
- **PowerShell では python の出力を `>`・`|`・`$x = ...` で受けない。** Windows PowerShell 5.1 では日本語が化け、`>` の
  ...
  長い出力を残したいときは `--out out/ファイル名`(UTF-8 BOM付きで保存、画面には `[OK]/[NG] 保存先(行数)` の1行だけ)を使う。
  `out/` は gitignore 済み。リポジトリ直下など別の場所に保存したファイルはコミットしない。
...
- 「特定の山だけで探索」したいときは `MOUNTAINS` を編集せず、**新しい引数**として `parse_args()` と `select_pool()` に
  条件を足す(引数を付けないときは従来どおり全山。`MOUNTAINS` の順番を変えると診断モードの番号がずれる)。
  山の座標(`lat`/`lon`)・地域(`region`)は各山の dict にある。選別条件を足したら、`main()` の `region_label`
  (`【対象地域:` の行)にもその条件が出るようにし、新しい引数のテストを追加する。
