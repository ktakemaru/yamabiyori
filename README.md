...
### v1.2.0 (2026-09-11)
- 標高別の値(雲量・RH・風速・風向・気温)を気圧面の線形補間方式に再設計し、1000/2000/3000m・各山頂標高の値に近づけた。雲海のような逆転層を高スコアと誤判定しないよう「山頂上空が乾燥していれば下の湿潤層は数えない」蓋判定を追加。
- 国土地理院DEMに基づく地形レイヤーを追加。山頂・稜線帯下端の風向と地形から風上/風下・地形性上昇流を表示する(現時点は表示のみでスコアには未反映)。
- 雨天判定をMSM降水量と湿潤層(RH・雲量)ベースに統一し、単発の雲量・降水スパイクで日スコアが0点になる問題を緩和(連続2時間平均・活動時間内合計mmを採用)。
- 診断モードに気象庁天気図・ひまわり画像を使った概況説明を追加(MSM範囲内)。
- 登録座標が山頂から300m以上外れていた12座を修正、キャッシュの時刻軸ズレなど基盤バグを修正。
